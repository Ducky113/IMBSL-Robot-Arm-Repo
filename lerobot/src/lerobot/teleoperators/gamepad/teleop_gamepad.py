...
class GamepadTeleop(Teleoperator):
    ...
    def action_features(self) -> dict:
        if self.config.use_gripper:
            ...
            return {
                "dtype": "float32",
                "shape": (4,),
                "names": {"delta_x": 0, "delta_y": 1, "delta_z": 2, "delta_wz": 3},
            }
    ...
    def connect(self) -> None:
        ...
        self.gamepad = Gamepad(
            wz_step_size=self.config.wz_step_size,
            gripper_open_button=self.config.gripper_open_button,
            gripper_close_button=self.config.gripper_close_button,
            gripper_open_trigger_axis=self.config.gripper_open_trigger_axis,
            gripper_close_trigger_axis=self.config.gripper_close_trigger_axis,
        )
        self.gamepad.start()
