...
class GamepadController(InputController):
    ...
    def __init__(
        ...
        self.gripper_open_trigger_axis = gripper_open_trigger_axis
        self.gripper_close_trigger_axis = gripper_close_trigger_axis
        self.joystick = None
        self.intervention_flag = False
        self.wrist_roll_mode = False

    @staticmethod
    def _trigger_pressed(axis_value: float, deadzone: float = 0.1) -> bool:
        """True when an analog trigger is pressed (handles -1..1 and 0..1 ranges)."""
        normalized = (axis_value + 1.0) / 2.0 if axis_value < 0.0 else axis_value
        return normalized > (0.5 + deadzone)
    ...
    def start(self):
        ...
        if self.gripper_open_button is None:
            print(f"  LT (axis {self.gripper_open_trigger_axis}): Open gripper")
        else:
            print(f"  Button {self.gripper_open_button}: Open gripper")
        if self.gripper_close_button is None:
            print(f"  RT (axis {self.gripper_close_trigger_axis}): Close gripper")
        else:
            print(f"  Button {self.gripper_close_button}: Close gripper")
    ...
    def update(self):
        ...
        for event in pygame.event.get():
            if event.type == pygame.JOYBUTTONDOWN:
                ...
                elif self.gripper_close_button is not None and event.button == self.gripper_close_button:
                    self.close_gripper_command = True
                elif self.gripper_open_button is not None and event.button == self.gripper_open_button:
        ...
        if self.gripper_open_button is None:
            self.open_gripper_command = self._trigger_pressed(
                self.joystick.get_axis(self.gripper_open_trigger_axis), self.deadzone
            )
        if self.gripper_close_button is None:
            self.close_gripper_command = self._trigger_pressed(
                self.joystick.get_axis(self.gripper_close_trigger_axis), self.deadzone
            )
